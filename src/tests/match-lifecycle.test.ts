...
describe('completion validation',()=>{
  it('accepts a coherent result',()=>expect(()=>validateCompletion({winner:'radiant',radiantScore:42,direScore:31,durationSeconds:2538,reason:'Operator confirmed'})).not.toThrow());
  it.each([{winner:'radiant',radiantScore:1,direScore:2,durationSeconds:100,reason:'x'},{winner:'dire',radiantScore:1,direScore:1,durationSeconds:100,reason:'x'},{winner:'dire',radiantScore:-1,direScore:2,durationSeconds:100,reason:'x'},{winner:'dire',radiantScore:1,direScore:2,durationSeconds:10,reason:'x'},{winner:'dire',radiantScore:1,direScore:2,durationSeconds:100,reason:''}] as any[])('rejects invalid result %#',(value)=>expect(()=>validateCompletion(value)).toThrow());
});
